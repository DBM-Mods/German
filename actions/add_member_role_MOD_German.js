module.exports = {
  ...
  <option value="6">Ausführen und actions fortsetzen</option>
  </select>
  <br>
  </div>
    ...
    async action(cache) {
      ...
      if(memberfind == "100" || memberfind == "101"){
        ...
        if (!server?.members) {
          ...
          if (data.errcmd === true){console.log("Server-Cache wurde nicht gefunden")}
          this.storeValue("Server-Cache wurde nicht gefunden", data.errs, this.evalMessage(data.errv, cache), cache)
